...
export default function ProductManagement({ storeId: scopedStoreId }: { storeId?: number }) {
  ...
  useEffect(() => { setPage(1); }, [debouncedSearch, scopedStoreId]);
  const catalog = trpc.inventory.productCatalog.useQuery({ storeId: scopedStoreId, page, pageSize, search: debouncedSearch });
  const [printQuantities, setPrintQuantities] = useState<Record<string, number>>({});
  const [labelJob, setLabelJob] = useState<LabelJob | null>(null);
  const [isPrintingLabels, setIsPrintingLabels] = useState(false);
  const labelPrintRef = useRef<HTMLDivElement>(null);
  const draftQuery = trpc.inventory.productDrafts.useQuery({ storeId: scopedStoreId ?? 1 }, { enabled: Boolean(scopedStoreId), staleTime: 60_000 });
  const saveDraft = trpc.inventory.saveProductDraft.useMutation({ onSuccess: result => { if (result.id) setDraftSaveStatus("Saved"); } });
  const deleteDraft = trpc.inventory.deleteProductDraft.useMutation();
  ...
  const persistDraft = (draft: ProductDraft) => {
    if (!draft.storeId || !draft.name.trim() && !draft.sku.trim() && !draft.reference.trim() && !draft.barcodes.length) return;
    setDraftSaveStatus("Saving…");
    saveDraft.mutate({ storeId: Number(draft.storeId), payload: draft });
  };
  const createProduct = trpc.inventory.createProduct.useMutation({
    onSuccess: async () => { if (scopedStoreId) await deleteDraft.mutateAsync({ storeId: scopedStoreId }); await refreshCatalog(); await utils.inventory.productDrafts.invalidate(); setCreateOpen(false); toast.success("Product created with its barcode list."); },
  ...
  const totalPages = Math.max(1, Math.ceil((catalog.data?.total ?? 0) / pageSize));
  const scopedStore = scopedStoreId ? stores.find(store => store.id === scopedStoreId) : undefined;
  const isCashier = Boolean(scopedStoreId && user && !user.isSuperAdmin && user.branchRoles.some(assignment => assignment.storeId === scopedStoreId && assignment.role === "cashier"));
  const canEditProduct = (product: ProductRecord) => !isCashier || product.createdByUserId === user?.id;
  ...
  useEffect(() => { const validIds = new Set(products.map(product => product.id)); setSelectedProductIds(current => { const next = current.filter(id => validIds.has(id)); return next.length === current.length ? current : next; }); }, [products]);
